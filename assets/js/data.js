// ===== Gitmaxhub Project Data =====
// Auto-updated daily. DO NOT manually edit between markers.
// Total projects: 37 (refreshed from GitHub API / Trending / OSSInsight)


// DATA_START
const UPDATE_DATE = '2026-09-05';

const PROJECTS = [
  {
    "slug": "openclaw",
    "name": "OpenClaw",
    "fullName": "openclaw/openclaw",
    "githubUrl": "https://github.com/openclaw/openclaw",
    "lang": "TypeScript",
    "stars": "388871",
    "forks": "81690",
    "category": "agent",
    "tags": [
      "AI Agent",
      "跨平台",
      "多端协同",
      "本地优先"
    ],
    "descShort": "GitHub历史星标第一，388.8k Stars，本地优先的跨平台AI助理框架",
    "descFull": "OpenClaw是GitHub历史上星标最高的开源项目，累计388871 Stars。完全运行在用户自己的设备上，原生连接WhatsApp/Telegram/Slack/Discord/飞书等50+渠道，数据不出设备，被Sam Altman公开背书，是本地优先架构的代表作。",
    "features": [
      "GitHub历史星标第一388.8k",
      "50+原生渠道集成",
      "完全本地自托管",
      "本地LLM兼容",
      "Docker一键部署"
    ]
  },
  {
    "slug": "superpowers",
    "name": "Superpowers",
    "fullName": "obra/superpowers",
    "githubUrl": "https://github.com/obra/superpowers",
    "lang": "Shell",
    "stars": "281807",
    "forks": "25239",
    "category": "agent",
    "tags": [
      "AI Agent",
      "Skills框架",
      "TDD",
      "开发方法论"
    ],
    "descShort": "Agentic Skills框架与方法论，281.4k Stars，TDD驱动的结构化工作流",
    "descFull": "Superpowers是Agentic Skills框架与软件开发方法论，累计281807 Stars。定义7阶段结构化工作流（需求澄清、计划审批、TDD实现、子Agent并行、审查收尾），测试先行铁律，已进入Claude Code和Codex官方插件市场。",
    "features": [
      "7阶段结构化工作流",
      "TDD铁律测试先行",
      "子Agent并行隔离",
      "Claude Code/Codex官方集成",
      "281.4k Stars"
    ]
  },
  {
    "slug": "hermes-agent",
    "name": "Hermes Agent",
    "fullName": "NousResearch/hermes-agent",
    "githubUrl": "https://github.com/NousResearch/hermes-agent",
    "lang": "Python",
    "stars": "241500",
    "forks": "49565",
    "category": "agent",
    "tags": [
      "AI Agent",
      "自我进化",
      "多网关",
      "持久记忆"
    ],
    "descShort": "自我进化型通用AI Agent框架，240.9k Stars，六周47k爆发式增长",
    "descFull": "Hermes Agent是Nous Research开源的自我进化通用Agent框架，累计241500 Stars。通过自我改进循环将成功经验编码为可复用Skills，支持跨会话持久记忆，集成CLI/Telegram/Discord/Slack等15+网关，OpenRouter接入200+模型，内置40+工具。",
    "features": [
      "自我改进循环",
      "跨会话持久记忆",
      "15+消息网关",
      "OpenRouter 200+模型",
      "40+内置工具"
    ]
  },
  {
    "slug": "autogpt",
    "name": "AutoGPT",
    "fullName": "Significant-Gravitas/AutoGPT",
    "githubUrl": "https://github.com/Significant-Gravitas/AutoGPT",
    "lang": "Python",
    "stars": "187132",
    "forks": "46043",
    "category": "agent",
    "tags": [
      "AI Agent",
      "自主执行",
      "任务分解",
      "可视化平台"
    ],
    "descShort": "老牌自主AI Agent平台，187.1k Stars，任务分解与自主执行标杆",
    "descFull": "AutoGPT是开创性的自主AI Agent项目，累计187132 Stars。以任务分解与自主执行为核心理念，提供可视化构建与运行自主工作流的平台，是「可访问AI」愿景的代表作，插件生态丰富。",
    "features": [
      "任务自主分解执行",
      "可视化Agent平台",
      "插件生态丰富",
      "支持多模型接入",
      "187.1k Stars"
    ]
  },
  {
    "slug": "opencode",
    "name": "opencode",
    "fullName": "anomalyco/opencode",
    "githubUrl": "https://github.com/anomalyco/opencode",
    "lang": "TypeScript",
    "stars": "204141",
    "forks": "26634",
    "category": "agent",
    "tags": [
      "Coding Agent",
      "终端",
      "TypeScript",
      "开源"
    ],
    "descShort": "开源Coding Agent最大项目，203.6k Stars，终端AI编码新霸主",
    "descFull": "opencode是当前开源Coding Agent中规模最大的项目，累计204141 Stars。开源许可、自主Agent架构、模块化工具集成（文件IO/Shell/测试），LLM无关设计支持多模型供应商，近28天新增超900 Stars。",
    "features": [
      "开源Coding Agent第一",
      "LLM无关设计",
      "模块化工具集成",
      "高增长势头",
      "203.6k Stars"
    ]
  },
  {
    "slug": "claude-code",
    "name": "Claude Code",
    "fullName": "anthropics/claude-code",
    "githubUrl": "https://github.com/anthropics/claude-code",
    "lang": "Python",
    "stars": "144095",
    "forks": "23018",
    "category": "agent",
    "tags": [
      "Coding Agent",
      "Anthropic",
      "终端",
      "自然语言"
    ],
    "descShort": "Anthropic官方终端编码Agent，144k Stars，自然语言操作代码库",
    "descFull": "Claude Code是Anthropic官方开源的终端Agent编码工具，累计144095 Stars。通过自然语言理解代码库并处理git工作流，支持Claude的extended thinking与computer use能力，近28天新增561 Stars。",
    "features": [
      "Anthropic官方出品",
      "自然语言代码库操作",
      "支持computer use",
      "与Claude深度集成",
      "144k Stars"
    ]
  },
  {
    "slug": "gemini-cli",
    "name": "Gemini CLI",
    "fullName": "google-gemini/gemini-cli",
    "githubUrl": "https://github.com/google-gemini/gemini-cli",
    "lang": "TypeScript",
    "stars": "106815",
    "forks": "14528",
    "category": "agent",
    "tags": [
      "Coding Agent",
      "Google",
      "终端",
      "Gemini"
    ],
    "descShort": "Google官方终端AI Agent，106.8k Stars，Gemini进终端",
    "descFull": "Gemini CLI是Google官方开源AI Agent，累计106815 Stars。将Gemini模型能力带入终端，支持多模态交互与代码任务，与Claude Code、OpenCode构成终端Agent三强格局中的Google代表。",
    "features": [
      "Google官方出品",
      "多模态终端交互",
      "代码任务支持",
      "Gemini生态集成",
      "106.8k Stars"
    ]
  },
  {
    "slug": "browser-use",
    "name": "browser-use",
    "fullName": "browser-use/browser-use",
    "githubUrl": "https://github.com/browser-use/browser-use",
    "lang": "Python",
    "stars": "112294",
    "forks": "12354",
    "category": "agent",
    "tags": [
      "浏览器自动化",
      "AI Agent",
      "网页操作",
      "Python"
    ],
    "descShort": "让AI Agent操作浏览器的核心库，112.2k Stars，网站自动化首选",
    "descFull": "browser-use是让AI Agent访问并操作任何网站的Python库，累计112294 Stars。18个月从发布冲到94k+ Stars，是浏览器自动化赛道winner-take-most的领军者，让任意网站对Agent可访问。",
    "features": [
      "网站Agent化访问",
      "18个月爆发增长",
      "自动化在线任务",
      "与主流Agent框架集成",
      "112.2k Stars"
    ]
  },
  {
    "slug": "openhands",
    "name": "OpenHands",
    "fullName": "OpenHands/OpenHands",
    "githubUrl": "https://github.com/OpenHands/OpenHands",
    "lang": "TypeScript",
    "stars": "86185",
    "forks": "11306",
    "category": "agent",
    "tags": [
      "Coding Agent",
      "软件开发",
      "自主Agent",
      "生产级"
    ],
    "descShort": "AI驱动的软件开发平台，86.1k Stars，自主编码Agent生产级落地",
    "descFull": "OpenHands是AI驱动的软件开发平台，累计86185 Stars。以自主编码Agent为核心，支持会话状态持久化、长任务checkpoint，覆盖从原型到生产的完整开发流程自动化。",
    "features": [
      "自主编码Agent",
      "会话状态持久化",
      "长任务checkpoint",
      "生产级部署",
      "86.1k Stars"
    ]
  },
  {
    "slug": "codex",
    "name": "Codex",
    "fullName": "openai/codex",
    "githubUrl": "https://github.com/openai/codex",
    "lang": "Rust",
    "stars": "121,558",
    "forks": "18,627",
    "category": "agent",
    "tags": [
      "Coding Agent",
      "OpenAI",
      "Rust",
      "终端"
    ],
    "descShort": "OpenAI官方终端编码Agent，121.6k Stars，轻量高性能Rust实现",
    "descFull": "Codex是OpenAI官方的轻量级终端编码Agent，累计121,558 Stars。以Rust实现追求极致性能，理解代码库、执行命令并自动完成编码任务，与Claude Code、opencode共同构成终端Coding Agent头部阵营，Apache-2.0开源。",
    "features": [
      "OpenAI官方出品",
      "Rust高性能实现",
      "终端原生Agent",
      "Apache-2.0开源",
      "121.6k Stars"
    ]
  },
  {
    "slug": "pi",
    "name": "Pi",
    "fullName": "earendil-works/pi",
    "githubUrl": "https://github.com/earendil-works/pi",
    "lang": "TypeScript",
    "stars": "101,845",
    "forks": "12,686",
    "category": "agent",
    "tags": [
      "AI Agent",
      "工具包",
      "统一LLM API",
      "TUI"
    ],
    "descShort": "一体化AI Agent工具包，101.8k Stars，统一LLM API+终端Coding CLI",
    "descFull": "Pi是一体化AI Agent工具包，累计101,845 Stars。统一LLM API、Agent主循环、TUI界面与Coding Agent CLI一站式提供，TypeScript实现，是2026年高增长的开源Agent基建项目。",
    "features": [
      "统一LLM API",
      "Agent主循环",
      "TUI界面",
      "Coding Agent CLI",
      "101.8k Stars"
    ]
  },
  {
    "slug": "scientific-agent-skills",
    "name": "Scientific Agent Skills",
    "fullName": "K-Dense-AI/scientific-agent-skills",
    "githubUrl": "https://github.com/K-Dense-AI/scientific-agent-skills",
    "lang": "Python",
    "stars": "42,644",
    "forks": "3,900",
    "category": "agent",
    "tags": [
      "Agent Skills",
      "科研",
      "AI Scientist",
      "Python"
    ],
    "descShort": "科研Agent Skills库第一，42.6k Stars，把任意Agent变成AI科学家",
    "descFull": "Scientific Agent Skills是科研领域排名第一的Agent Skills库，累计42,644 Stars。内置文献阅读、实验设计、数据分析、论文撰写等科研技能，让任意AI Agent化身AI Scientist，已有190,000+科研人员使用，MIT开源。",
    "features": [
      "190k+科研用户",
      "科研全流程Skills",
      "任意Agent可接入",
      "MIT开源",
      "42.6k Stars"
    ]
  },
  {
    "slug": "dify",
    "name": "Dify",
    "fullName": "langgenius/dify",
    "githubUrl": "https://github.com/langgenius/dify",
    "lang": "TypeScript",
    "stars": "154473",
    "forks": "24414",
    "category": "tool",
    "tags": [
      "LLM平台",
      "低代码",
      "RAG",
      "工作流"
    ],
    "descShort": "生产级Agentic AI应用平台，154.4k Stars，低代码LLM应用王者",
    "descFull": "Dify是生产级的Agentic AI应用开发平台，累计154473 Stars。可视化工作流构建器+RAG流水线+API层一体，是低代码AI开发的事实标准，被广泛用于企业级LLM应用搭建。",
    "features": [
      "可视化工作流构建",
      "内置RAG流水线",
      "API层一体化",
      "企业级应用",
      "154.4k Stars"
    ]
  },
  {
    "slug": "n8n",
    "name": "n8n",
    "fullName": "n8n-io/n8n",
    "githubUrl": "https://github.com/n8n-io/n8n",
    "lang": "TypeScript",
    "stars": "203373",
    "forks": "60558",
    "category": "tool",
    "tags": [
      "工作流自动化",
      "低代码",
      "自托管",
      "AI集成"
    ],
    "descShort": "工作流自动化平台，203.3k Stars，原生AI能力的n8n",
    "descFull": "n8n是工作流自动化平台，累计203373 Stars。原生集成AI能力，400+集成节点，支持自托管，是「2026年大量Agent工作负载实为带LLM节点的工作流自动化」趋势的最大受益者。",
    "features": [
      "400+集成节点",
      "原生AI能力",
      "自托管部署",
      "可视化工作流",
      "203.3k Stars"
    ]
  },
  {
    "slug": "firecrawl",
    "name": "Firecrawl",
    "fullName": "firecrawl/firecrawl",
    "githubUrl": "https://github.com/firecrawl/firecrawl",
    "lang": "TypeScript",
    "stars": "176536",
    "forks": "9660",
    "category": "tool",
    "tags": [
      "网页抓取",
      "LLM数据",
      "API",
      "数据管道"
    ],
    "descShort": "网页转LLM数据接口，176.2k Stars，Web数据抓取与交互API",
    "descFull": "Firecrawl是网页抓取与交互API，累计176536 Stars。将任意网站转换为LLM-ready数据，支持实时Web数据获取、抓取、爬取与结构化输出，是Agent与网页数据之间的关键桥梁。",
    "features": [
      "网页转LLM数据",
      "实时数据获取",
      "结构化输出",
      "Agent数据接口",
      "176.2k Stars"
    ]
  },
  {
    "slug": "ragflow",
    "name": "RAGFlow",
    "fullName": "infiniflow/ragflow",
    "githubUrl": "https://github.com/infiniflow/ragflow",
    "lang": "Go",
    "stars": "90054",
    "forks": "10621",
    "category": "tool",
    "tags": [
      "RAG",
      "知识库",
      "检索增强",
      "引用溯源"
    ],
    "descShort": "深度RAG引擎，90k Stars，智能检索+引用溯源",
    "descFull": "RAGFlow是带Agent能力的深度RAG引擎，累计90054 Stars。智能检索+引用溯源+支持混乱真实文档，是企业级知识库的superior context layer，是文档密集型场景的首选。",
    "features": [
      "深度RAG引擎",
      "引用溯源",
      "智能Agent检索",
      "企业知识库",
      "90k Stars"
    ]
  },
  {
    "slug": "mem0",
    "name": "Mem0",
    "fullName": "mem0ai/mem0",
    "githubUrl": "https://github.com/mem0ai/mem0",
    "lang": "Python",
    "stars": "64707",
    "forks": "7580",
    "category": "tool",
    "tags": [
      "Agent记忆",
      "上下文管理",
      "持久化",
      "记忆层"
    ],
    "descShort": "AI Agent通用记忆层，64.7k Stars，持久化上下文",
    "descFull": "Mem0是AI Agent的通用记忆层，累计64707 Stars。为Agent提供持久化上下文与跨会话记忆，被广泛集成到各类Agent框架中，是Agent记忆赛道的领跑者。",
    "features": [
      "通用记忆层",
      "跨会话上下文",
      "框架集成广泛",
      "Apache-2.0",
      "64.7k Stars"
    ]
  },
  {
    "slug": "llama-index",
    "name": "LlamaIndex",
    "fullName": "run-llama/llama_index",
    "githubUrl": "https://github.com/run-llama/llama_index",
    "lang": "Python",
    "stars": "52023",
    "forks": "8080",
    "category": "tool",
    "tags": [
      "RAG",
      "数据框架",
      "文档处理",
      "索引检索"
    ],
    "descShort": "数据框架连接LLM，52k Stars，生产RAG标准",
    "descFull": "LlamaIndex是连接LLM与数据的领先框架，累计52023 Stars。强大的索引与检索能力，Workflows支持序列化恢复的会话状态，是生产RAG标准与文档Agent平台。",
    "features": [
      "文档Agent平台",
      "生产RAG标准",
      "索引检索能力强",
      "Workflows状态恢复",
      "52k Stars"
    ]
  },
  {
    "slug": "gpt-researcher",
    "name": "GPT Researcher",
    "fullName": "assafelovic/gpt-researcher",
    "githubUrl": "https://github.com/assafelovic/gpt-researcher",
    "lang": "Python",
    "stars": "29291",
    "forks": "3981",
    "category": "tool",
    "tags": [
      "Deep Research",
      "研究Agent",
      "报告生成",
      "多源检索"
    ],
    "descShort": "自主深度研究Agent，29.3k Stars，联网调研自动生成报告",
    "descFull": "GPT Researcher是自主深度研究Agent，累计29291 Stars。自动规划搜索、多源信息交叉验证并生成结构化研究报告，是Deep Research类工具的开源代表。",
    "features": [
      "自主研究规划",
      "多源信息交叉",
      "结构化报告生成",
      "深度调研",
      "29.3k Stars"
    ]
  },
  {
    "slug": "lightrag",
    "name": "LightRAG",
    "fullName": "HKUDS/LightRAG",
    "githubUrl": "https://github.com/HKUDS/LightRAG",
    "lang": "Python",
    "stars": "39400",
    "forks": "5546",
    "category": "tool",
    "tags": [
      "RAG",
      "轻量级",
      "图检索",
      "EMNLP"
    ],
    "descShort": "简单快速的RAG框架，39.4k Stars，EMNLP 2025成果",
    "descFull": "LightRAG是来自EMNLP 2025的简单快速检索增强生成框架，累计39400 Stars。以极简API实现高效RAG，支持增量索引与图结构检索，是轻量级RAG方案的优选。",
    "features": [
      "极简API",
      "图结构检索",
      "增量索引",
      "EMNLP 2025",
      "39.4k Stars"
    ]
  },
  {
    "slug": "docling",
    "name": "Docling",
    "fullName": "docling-project/docling",
    "githubUrl": "https://github.com/docling-project/docling",
    "lang": "Python",
    "stars": "66007",
    "forks": "4753",
    "category": "tool",
    "tags": [
      "文档处理",
      "数据管道",
      "解析",
      "生成式AI"
    ],
    "descShort": "文档预处理管道，66k Stars，为生成式AI准备文档",
    "descFull": "Docling是文档预处理工具，累计66007 Stars。将PDF/Word/PPT等复杂文档解析为结构化数据，为生成式AI管道准备干净输入，由IBM开源并广泛采用。",
    "features": [
      "复杂文档解析",
      "生成式AI管道",
      "多格式支持",
      "IBM开源",
      "66k Stars"
    ]
  },
  {
    "slug": "langfuse",
    "name": "Langfuse",
    "fullName": "langfuse/langfuse",
    "githubUrl": "https://github.com/langfuse/langfuse",
    "lang": "TypeScript",
    "stars": "34208",
    "forks": "3696",
    "category": "tool",
    "tags": [
      "可观测性",
      "LLMOps",
      "追踪",
      "评估"
    ],
    "descShort": "LLM可观测性平台，34.2k Stars，Agent追踪与评估",
    "descFull": "Langfuse是开源LLM可观测性平台，累计34208 Stars。提供Agent/LLM应用的追踪、监控、评估与提示管理，是生产级Agent应用的观测层标配。",
    "features": [
      "LLM追踪",
      "可观测性",
      "评估与提示管理",
      "开源自托管",
      "34.2k Stars"
    ]
  },
  {
    "slug": "litellm",
    "name": "LiteLLM",
    "fullName": "BerriAI/litellm",
    "githubUrl": "https://github.com/BerriAI/litellm",
    "lang": "Python",
    "stars": "58057",
    "forks": "11175",
    "category": "tool",
    "tags": [
      "LLM网关",
      "API路由",
      "多模型",
      "统一接口"
    ],
    "descShort": "统一LLM网关，511.2k Stars，200+模型一个API",
    "descFull": "LiteLLM是通用LLM API网关，累计58057 Stars。一个统一接口接入200+提供商，支持智能路由、自动回退与配额管理，是生产级多模型架构的标准组件。",
    "features": [
      "200+提供商统一接口",
      "智能路由",
      "自动回退",
      "配额管理",
      "511.2k Stars"
    ]
  },
  {
    "slug": "comfyui",
    "name": "ComfyUI",
    "fullName": "Comfy-Org/ComfyUI",
    "githubUrl": "https://github.com/Comfy-Org/ComfyUI",
    "lang": "Python",
    "stars": "131536",
    "forks": "15505",
    "category": "multimodal",
    "tags": [
      "图像生成",
      "视频生成",
      "节点式",
      "扩散模型"
    ],
    "descShort": "节点式AI图像/视频生成界面，131.4k Stars，模块化扩散模型GUI",
    "descFull": "ComfyUI是模块化扩散模型GUI与后端，累计131536 Stars。图节点式接口构建生成式图像与视频工作流，插件生态海量，是AIGC创作基础设施的核心。",
    "features": [
      "节点式工作流",
      "图像视频生成",
      "插件生态海量",
      "模块化后端",
      "131.4k Stars"
    ]
  },
  {
    "slug": "whisper",
    "name": "Whisper",
    "fullName": "openai/whisper",
    "githubUrl": "https://github.com/openai/whisper",
    "lang": "Python",
    "stars": "108445",
    "forks": "13135",
    "category": "multimodal",
    "tags": [
      "语音识别",
      "ASR",
      "多语言",
      "OpenAI"
    ],
    "descShort": "OpenAI语音识别模型，108.4k Stars，通用语音识别标准",
    "descFull": "Whisper是OpenAI开源的通用语音识别模型，累计108445 Stars。多语言、多任务（识别/翻译/转录），是语音Agent与音频处理领域的事实标准基础模型。",
    "features": [
      "通用语音识别",
      "多语言支持",
      "多任务能力",
      "语音Agent基础",
      "108.4k Stars"
    ]
  },
  {
    "slug": "open-sora",
    "name": "Open-Sora",
    "fullName": "hpcaitech/Open-Sora",
    "githubUrl": "https://github.com/hpcaitech/Open-Sora",
    "lang": "Python",
    "stars": "29352",
    "forks": "3028",
    "category": "multimodal",
    "tags": [
      "视频生成",
      "扩散模型",
      "DiT",
      "开源Sora"
    ],
    "descShort": "开源视频生成星标最高，29.4k Stars，Sora架构复现",
    "descFull": "Open-Sora是GitHub星标最高的开源视频生成项目，累计29352 Stars。2.0版本（11B参数）在VBench达到与HunyuanVideo相当性能，训练成本约20万美元，支持文生视频、图生视频与无限长度生成。",
    "features": [
      "Sora架构DiT",
      "文生图生视频",
      "无限长度生成",
      "训练可复现",
      "29.4k Stars"
    ]
  },
  {
    "slug": "vibevoice",
    "name": "VibeVoice",
    "fullName": "microsoft/VibeVoice",
    "githubUrl": "https://github.com/microsoft/VibeVoice",
    "lang": "Python",
    "stars": "53692",
    "forks": "6066",
    "category": "multimodal",
    "tags": [
      "语音AI",
      "语音合成",
      "微软",
      "声音智能"
    ],
    "descShort": "微软开源前沿语音AI，53.7k Stars，开源声音智能",
    "descFull": "VibeVoice是微软开源的前沿语音AI项目，累计53692 Stars。覆盖语音识别、合成与对话能力，是2026年开源语音赛道的新势力。",
    "features": [
      "前沿语音AI",
      "微软开源",
      "语音识别合成",
      "对话语音",
      "53.7k Stars"
    ]
  },
  {
    "slug": "speech-to-speech",
    "name": "Speech-to-Speech",
    "fullName": "huggingface/speech-to-speech",
    "githubUrl": "https://github.com/huggingface/speech-to-speech",
    "lang": "Python",
    "stars": "13040",
    "forks": "1637",
    "category": "multimodal",
    "tags": [
      "语音Agent",
      "语音交互",
      "本地部署",
      "HuggingFace"
    ],
    "descShort": "本地语音Agent构建工具，13k Stars，开源模型语音交互",
    "descFull": "Speech-to-Speech是Hugging Face开源的本地语音Agent构建框架，累计13040 Stars。用开源模型在本地构建语音到语音的实时对话Agent，是语音交互赛道的重要基建。",
    "features": [
      "本地语音Agent",
      "开源模型",
      "实时对话",
      "HuggingFace出品",
      "13k Stars"
    ]
  },
  {
    "slug": "openmontage",
    "name": "OpenMontage",
    "fullName": "calesthio/OpenMontage",
    "githubUrl": "https://github.com/calesthio/OpenMontage",
    "lang": "Python",
    "stars": "56,156",
    "forks": "7,025",
    "category": "multimodal",
    "tags": [
      "视频生成",
      "Agentic视频",
      "生产流水线",
      "Python"
    ],
    "descShort": "首个开源Agentic视频生产系统，56.2k Stars，12条生产流水线",
    "descFull": "OpenMontage是全球首个开源的Agentic视频生产系统，累计56,156 Stars。内置12条生产流水线、100+创作工具、700+模型支持，覆盖从脚本、分镜到成片的自动化视频创作全流程，AGPL-3.0开源。",
    "features": [
      "12条生产流水线",
      "100+创作工具",
      "700+模型支持",
      "Agentic视频生产",
      "56.2k Stars"
    ]
  },
  {
    "slug": "transformers",
    "name": "Transformers",
    "fullName": "huggingface/transformers",
    "githubUrl": "https://github.com/huggingface/transformers",
    "lang": "Python",
    "stars": "164794",
    "forks": "34448",
    "category": "framework",
    "tags": [
      "机器学习",
      "多模态",
      "HuggingFace",
      "模型库"
    ],
    "descShort": "HuggingFace核心ML框架，164.8k Stars，多模态模型标准",
    "descFull": "Transformers是HuggingFace的State-of-the-Art机器学习框架，累计164794 Stars。支持文本、视觉、音频与多模态模型，是开源ML生态的基石。",
    "features": [
      "多模态模型支持",
      "SOTA模型库",
      "生态基石",
      "社区海量模型",
      "164.8k Stars"
    ]
  },
  {
    "slug": "langchain",
    "name": "LangChain",
    "fullName": "langchain-ai/langchain",
    "githubUrl": "https://github.com/langchain-ai/langchain",
    "lang": "Python",
    "stars": "145659",
    "forks": "24316",
    "category": "framework",
    "tags": [
      "LLM框架",
      "Agent编排",
      "生态标准",
      "Python"
    ],
    "descShort": "LLM应用工程平台，145.6k Stars，Agent生态标准",
    "descFull": "LangChain是Agent工程平台与生态标准，累计145659 Stars。模块化架构覆盖LLM应用全链路，生态集成数量无出其右，2026年新增Claude adaptive thinking与OpenAI Responses API原生支持。",
    "features": [
      "模块化架构",
      "生态集成最广",
      "Agent编排",
      "原生流式支持",
      "145.6k Stars"
    ]
  },
  {
    "slug": "ollama",
    "name": "Ollama",
    "fullName": "ollama/ollama",
    "githubUrl": "https://github.com/ollama/ollama",
    "lang": "Go",
    "stars": "180167",
    "forks": "17695",
    "category": "framework",
    "tags": [
      "本地LLM",
      "推理运行时",
      "边缘AI",
      "私有部署"
    ],
    "descShort": "本地LLM运行时，180.1k Stars，50+模型一条命令",
    "descFull": "Ollama是本地LLM运行时，累计180167 Stars。支持50+模型一键运行，是边缘AI民主化的代表，本地部署LLM的事实标准。",
    "features": [
      "50+模型支持",
      "一条命令运行",
      "本地私有部署",
      "边缘AI",
      "180.1k Stars"
    ]
  },
  {
    "slug": "llama-cpp",
    "name": "llama.cpp",
    "fullName": "ggml-org/llama.cpp",
    "githubUrl": "https://github.com/ggml-org/llama.cpp",
    "lang": "C++",
    "stars": "127076",
    "forks": "22768",
    "category": "framework",
    "tags": [
      "LLM推理",
      "C++",
      "本地部署",
      "量化"
    ],
    "descShort": "高性能LLM推理引擎，127k Stars，CPU/边缘推理之王",
    "descFull": "llama.cpp是GGML组织的高性能LLM推理引擎，累计127076 Stars。C++实现、无GPU也能高效运行LLM，近28天新增394 Stars，是本地推理的核心基建。",
    "features": [
      "C++高性能",
      "CPU友好",
      "量化支持",
      "边缘设备推理",
      "127k Stars"
    ]
  },
  {
    "slug": "vllm",
    "name": "vLLM",
    "fullName": "vllm-project/vllm",
    "githubUrl": "https://github.com/vllm-project/vllm",
    "lang": "Python",
    "stars": "90981",
    "forks": "21714",
    "category": "framework",
    "tags": [
      "LLM推理",
      "高吞吐",
      "生产部署",
      "服务引擎"
    ],
    "descShort": "高吞吐LLM推理引擎，90.9k Stars，生产部署关键",
    "descFull": "vLLM是高吞吐、内存高效的LLM推理引擎，累计90981 Stars。PagedAttention等创新技术使其成为生产环境LLM服务的关键基础设施，广泛用于模型服务。",
    "features": [
      "高吞吐推理",
      "PagedAttention",
      "内存高效",
      "生产级服务",
      "90.9k Stars"
    ]
  },
  {
    "slug": "crewai",
    "name": "CrewAI",
    "fullName": "crewAIInc/crewAI",
    "githubUrl": "https://github.com/crewAIInc/crewAI",
    "lang": "Python",
    "stars": "58093",
    "forks": "8339",
    "category": "framework",
    "tags": [
      "多Agent",
      "角色编排",
      "协作框架",
      "Python"
    ],
    "descShort": "角色扮演多Agent框架，58.1k Stars，团队协作式编排",
    "descFull": "CrewAI是角色驱动的多Agent编排框架，累计58093 Stars。将Agent定义为带角色/目标/背景故事的团队成员，顺序或层级协作完成任务，是快速搭建多Agent应用的首选，1500+企业采用。",
    "features": [
      "角色驱动设计",
      "任务委派协作",
      "层级协调",
      "1500+企业采用",
      "58.1k Stars"
    ]
  },
  {
    "slug": "langgraph",
    "name": "LangGraph",
    "fullName": "langchain-ai/langgraph",
    "githubUrl": "https://github.com/langchain-ai/langgraph",
    "lang": "Python",
    "stars": "41067",
    "forks": "6936",
    "category": "framework",
    "tags": [
      "Agent编排",
      "状态图",
      "LangChain",
      "生产级"
    ],
    "descShort": "有状态图Agent框架，41k Stars，生产默认编排",
    "descFull": "LangGraph是LangChain的有状态图Agent框架，累计41067 Stars。图节点边建模、内置checkpoint、暂停/恢复/时间旅行调试，34.5M月下载量，是企业生产Agent的默认选择。",
    "features": [
      "图结构编排",
      "内置checkpoint",
      "时间旅行调试",
      "34.5M月下载",
      "41k Stars"
    ]
  },
  {
    "slug": "llama-factory",
    "name": "LlamaFactory",
    "fullName": "hiyouga/LlamaFactory",
    "githubUrl": "https://github.com/hiyouga/LlamaFactory",
    "lang": "Python",
    "stars": "74,579",
    "forks": "9,141",
    "category": "framework",
    "tags": [
      "微调",
      "LLM训练",
      "LoRA",
      "高效调优"
    ],
    "descShort": "统一高效微调框架，74.6k Stars，100+模型一键微调",
    "descFull": "LlamaFactory是统一的高效LLM/VLM微调框架，累计74,579 Stars。支持100+开源模型的LoRA/QLoRA等高效微调，覆盖预训练、指令微调、偏好对齐全流程（ACL 2024论文），是学术界与工业界微调的事实标准工具。",
    "features": [
      "100+模型支持",
      "LoRA/QLoRA",
      "全流程微调",
      "ACL 2024论文",
      "74.6k Stars"
    ]
  }
];

const CATEGORIES = {
  "agent": {
    "name": "Agent",
    "icon": "bot",
    "color": "#6366f1",
    "count": 12
  },
  "tool": {
    "name": "工具",
    "icon": "wrench",
    "color": "#10b981",
    "count": 11
  },
  "multimodal": {
    "name": "多模态",
    "icon": "image",
    "color": "#ec4899",
    "count": 6
  },
  "framework": {
    "name": "框架",
    "icon": "layers",
    "color": "#f59e0b",
    "count": 8
  },
};

const TRENDS = [
  {
    "title": "opencode登顶Coding Agent：194.5k Stars成开源编码第一",
    "desc": "anomalyco/opencode以194.5k Stars超越Claude Code（140.5k）与Gemini CLI（106.4k），成为开源Coding Agent最大项目。终端AI编码工作流全面Agent化，近28天新增超900 Stars。"
  },
  {
    "title": "OpenClaw稳居GitHub第一，本地优先成为主流",
    "desc": "OpenClaw 386.8k Stars持续霸榜，完全本地运行、50+原生渠道接入。与n8n 199.7k、Superpowers 274.4k一起，本地优先与工作流自动化成为2026年AI开源的两大主线。"
  },
  {
    "title": "低代码LLM平台双雄：Dify与Langflow齐破150k",
    "desc": "Dify 152.7k与Langflow 153.5k双双站稳150k+ Stars，可视化工作流构建成为企业LLM应用标配。低代码不是玩具，而是AI应用从原型到生产的快速通道。"
  },
  {
    "title": "本地推理全面爆发：Ollama、llama.cpp与vLLM三线推进",
    "desc": "Ollama 147.8k+llama.cpp 90.4k+vLLM 77.1k，本地与边缘推理从极客玩具变成生产级需求。隐私、成本、可控性三因素持续推动部署形态变革。"
  },
  {
    "title": "Agent记忆与可观测性成为生产必备基建",
    "desc": "Mem0 62.7k提供通用记忆层+Langfuse 32.7k提供追踪评估，Agent从「能跑」走向「可运维」。跨会话记忆、可观测性与评估体系成为生产级Agent的三件套。"
  },
  {
    "title": "语音赛道升温：VibeVoice、speech-to-speech与Whisper共振",
    "desc": "微软VibeVoice 33.4k+HuggingFace speech-to-speech 36.7k+OpenAI Whisper 106.8k，语音识别、合成与对话Agent全线开源化，语音交互进入Agent原生时代。"
  },
  {
    "title": "视频生成开源成熟：Open-Sora以20万美元训练成本挑战闭源",
    "desc": "Open-Sora 24.1k（11B参数，VBench对标HunyuanVideo）+ComfyUI 128.5k节点式生态，开源视频生成从实验室走向创作者工具，训练成本大幅下降成为关键拐点。"
  },
  {
    "title": "Deep Research工具链成型：从检索到报告全流程自动化",
    "desc": "GPT Researcher 28.9k自主调研+LightRAG 24k轻量RAG+RAGFlow 78.4k深度RAG，研究型Agent覆盖多源检索、图结构索引与引用溯源，深度调研进入自动化时代。"
  },
  {
    "title": "终端Agent三强格局确立：opencode/Claude Code/Gemini CLI",
    "desc": "opencode 194.5k、Claude Code 140.5k、Gemini CLI 106.4k形成终端Agent三强，另有OpenAI Codex CLI 104.5k紧随。终端从编码工具进化为完整的工作操作系统。"
  },
  {
    "title": "框架分化加剧：LangGraph生产默认、CrewAI快速原型、smolagents极简派",
    "desc": "LangGraph 39.8k以34.5M月下载成为生产默认；CrewAI 57.2k角色编排主打快速落地；HuggingFace smolagents以千行级极简代码另辟蹊径。热榜不再比谁家模型强，而是比编排与工程化能力。"
  },
];
// DATA_END
